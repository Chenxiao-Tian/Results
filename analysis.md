...
> 真实成功公司的平均 Overall 评分（7.92）仅比真实失败公司（7.74）高 0.18，且失败公司的中位数（8.19）仍然偏高，说明评分对真实结果的区分度有限。
...
## Probability 说明
多种方式尝试解析 PDF（包括直接解压对象流并查找含 `Probability` 的文本、手动 Flate 解码内容流、扫描内嵌字体表）均未发现可提取的文字数据；内容流主要是绘图/位图命令，文件本质上相当于嵌入图片而非可搜索文本。由于外网受限无法安装 OCR 或 PDF 解析工具，当前无法从 42 份 PDF 中逐一提取 “Quantitative Decision” 下方的 Probability 数值。如需补充，请提供可访问的文本版数据或允许使用带 OCR/PDF 文字提取能力的工具。
